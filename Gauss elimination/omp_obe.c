...
int main(){
    ...
    fp = fopen("result_omp.txt", "w"); // open file
    if (fp == NULL){ 
...
#define A(y, x) (*mat_elem(a, y, x, n))
	...
	for (dia = 0; dia < n; dia++) {
        ...
        #pragma omp parallel for shared(max, max_row) private(row)
		...
		for (row = dia + 1; row < n; row++) {
            ...
            #pragma omp parallel for shared(dia, n, tmp) private(col)
    ...
    #pragma omp parallel for shared (b, n, x) private(row)
